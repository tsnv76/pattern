...
from logs.framework_log import LOGGER
...
class Framework:
    ...
    def __call__(self, environ, start_response):
        ...
        if method == 'POST':
            ...
            LOGGER.debug(f'Нам пришёл post-запрос: {Framework.decode_value(data)}')
        if method == 'GET':
...
class DebugApplication(Framework):
    ...
    def __init__(self, routes_obj):
        self.application = Framework(routes_obj)
...
class FakeApplication(Framework):
    ...
    def __call__(self, environ, start_response):
        ...
        return [b'200 OK. Hello from Fake']